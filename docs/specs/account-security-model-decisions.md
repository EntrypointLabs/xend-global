...
signer set. `MAX_TIME_LOCK` is 3 months. **Set to 24 hours** (O4, resolved).
...
pending for the duration and the Consumer is notified. Without it a stolen quorum
rewrites the signer set instantly and silently, and the Consumer's own keys stop
working with no warning.

**Corrected 2026-09-07: a lone remaining signer cannot reject it.** The program
settles a proposal on this Account at two votes either way, so the rejection cutoff
is two signers and a single rejection leaves the change open (the app signs a
rejection with S2 and then S1 for exactly this reason, `useRejectAccountChange` in
`apps/mobile/hooks/usePendingAccountChange.ts`). A Consumer holding one signer against
a stolen pair can only watch the lock run down. This is an accepted risk. The
mitigation is the support freeze of S3's release (ADR 0030): while a report is open
the vault refuses to sign, so an attacker who needs S3 for their second vote never
gets it. Freezing is the mandatory first step in
[`multisig-credentials-runbook.md`](multisig-credentials-runbook.md), section 8.
It does not help against a stolen S1 plus S2, which is two of the Consumer's own
anchors and outside what this design defends.
...
> **Superseded on 2026-08-30 by [ADR 0027](../adr/0027-email-is-an-entry-point-not-a-login-method.md),
> as to ordering only.** The address is now proved first and S3 is minted against it
> before the passkey creates S1. D4 constrains what an email may _unlock_, not when it
> is _collected_, and the two were conflated here. The anchors, the table above and the
> invariant are unchanged. The rule the ordering has to respect: **email proves the
> inbox and unlocks S3, it never returns S1 and never enrols a signer.** One email,
> still once.
...
S2 alone cannot reject it (see the D3 correction: the rejection cutoff is two
signers); what stops it is the support freeze of S3's release, applied before S3's
vote is asked for. That is a survivable failure rather than an instant drain, and it
is why D10's platform rule can be relaxed.
...
That makes the notified window during the time lock a required control for money
movement, not only for signer-set changes. A Consumer who cannot see a staged
Settings transaction and get S3's release frozen has no protection here at all
(cancelling it alone is not possible; see the D3 correction).
...
| O7  | **Closed 2026-08-30.** First half resolved: there is no out-of-band MFA reset (see below). Second half, whether disabling email login app-wide blocks users who already have email linked, was never answered by Privy and is **closed as irrelevant rather than answered**: no Consumer exists in production, and stranded internal test accounts are re-created. One sequencing consequence stands: the dApp Store reviewer's ~0.75 USDC sits on a mainnet Privy wallet whose only way in may be email, so **run the sweep before disabling the method**. See [ADR 0027](../adr/0027-email-is-an-entry-point-not-a-login-method.md).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
...
| O10 | **Open, and it is the distance between D4 and the code.** D4's table says email does not unlock S1. As built it still does: the email OTP route signs in through Privy and returns the same embedded wallet that is S1, confirmed on device 2026-08-24. Reach, worst first. Spending never goes through the threshold at all (`spend.ts`: one signature from the primary signer, executed under the limit's policy), so an inbox reaches the whole daily spending limit with no second approval and no time lock, and rejecting settings changes does nothing about it. The signer set is still intact today, because S1 alone is one vote against a threshold of 2 and nothing calls `vault.open()`, so an inbox does not yet reach S3. The day S3's release is wired to a proof of that same inbox, one mailbox holds S1 and S3 together, which is the precise outcome D4 was rewritten to prevent. **Closing condition: passkey-only sign-in, with email login removed rather than demoted.** Blocked on Privy, who answer `passkeys/register/init` with a 200 and an empty body, so `signupWithPasskey` cannot complete and there is no way to create an account without email. Until that clears, email login is a migration route for Consumers who already exist and must not reach general availability. Ties to O7. **Updated 2026-08-30:** the Privy blocker cleared (passkey sign-up works, XEN-29 did not reproduce), so this is no longer blocked on a vendor. [ADR 0027](../adr/0027-email-is-an-entry-point-not-a-login-method.md) sets the closing path: email becomes an entry point that unlocks S3 and opens a limited, server-enforced session that cannot Spend, cannot change the signer set and cannot enrol a passkey. `(auth)/email-login.tsx` and the Privy email method come out with it. O10 closes when they are gone, not when they are demoted. **Closed 2026-08-30.** `(auth)/email-login.tsx`, `restore-account.tsx` and the Privy email login path came out in commit `b6dc884`; the entry session tier that replaces it landed in `d8dfb08` (ADR 0028). An emailed code now opens an `entry` session with no Privy session behind it, closed by default to every route that spends, changes the signer set or enrols a credential (`apps/backend/src/auth/entry-route-inventory.spec.ts`). |
...
This resolved O7's **first** half. The second half is closed by decision on
2026-08-30 rather than by an answer: see the O7 row in Open, and
[ADR 0027](../adr/0027-email-is-an-entry-point-not-a-login-method.md).
...
> **Extended 2026-08-30 by [ADR 0027](../adr/0027-email-is-an-entry-point-not-a-login-method.md).**
> Changing the contact address is a signer rotation, not an edit: one door, a settings
> change, the 24 hour lock, and the entry point moves on execution rather than on
> staging. Proof of the old address was considered and rejected, because a compromised
> inbox proves itself. Two risks are recorded there as accepted rather than solved: a
> second recovery key stays prompted rather than required, and the Gmail-on-Android
> overlap of S1's and S3's anchors is answered by the staged-change push alone.
