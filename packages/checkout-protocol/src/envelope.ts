...
export const CheckoutUnresolvedEnvelopeSchema = z.object({
  xend: z.literal('checkout'),
  v: z.literal(CHECKOUT_PROTOCOL_VERSION),
  nonce: z.string().min(1),
  reference: z.string().min(1),
  type: z.literal(CheckoutMessageType.Unresolved),
  reason: z.literal('confirmation_timeout'),
}) satisfies z.ZodType<CheckoutUnresolvedEnvelope>;
 ...
 *
 * The envelope shapes come back as themselves, discriminated by `type`, so
...
export function parseCheckoutMessage(
  ...
  const unresolved = CheckoutUnresolvedEnvelopeSchema.safeParse(event.data);
  if (unresolved.success) {
    if (unresolved.data.nonce !== expected.nonce) return null;
    if (unresolved.data.reference !== expected.reference) return null;
