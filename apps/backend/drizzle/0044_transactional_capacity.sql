...
SELECT 'cap:consumer:' || consumer_id || ':day:' || to_char(authorized_at, 'YYYYMMDD'),
...
GROUP BY consumer_id, date_trunc('day', authorized_at), to_char(authorized_at, 'YYYYMMDD');
--> statement-breakpoint
INSERT INTO capacity_counters (key, count, total_raw, expires_at)
SELECT 'cap:consumer:' || consumer_id || ':month:' || to_char(authorized_at, 'YYYYMM'),
       count(*)::integer, sum(usdc_settlement_raw::numeric),
...
GROUP BY consumer_id, date_trunc('month', authorized_at), to_char(authorized_at, 'YYYYMM');
