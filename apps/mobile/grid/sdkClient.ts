...
export class SDKGridClient {
  private static validateBackendEnv() {
    if (!process.env.EXPO_PUBLIC_GRID_ENDPOINT || !process.env.GRID_API_KEY) {
      throw new Error(
        "Missing required backend environment variables: EXPO_PUBLIC_GRID_ENDPOINT and GRID_API_KEY must be set."
      );
    }
  }
  ...
  private static validateFrontendEnv() {
    if (!process.env.EXPO_PUBLIC_GRID_ENDPOINT) {
      throw new Error(
        "Missing required frontend environment variable: EXPO_PUBLIC_GRID_ENDPOINT must be set."
      );
    }
  }
  ...
  private static getEnvironment(): GridEnvironment {
    ...
    if (!gridEnv) {
      throw new Error(
        "EXPO_PUBLIC_GRID_ENV environment variable must be set (sandbox or production)."
      );
    }
    if (gridEnv !== "sandbox" && gridEnv !== "production") {
      throw new Error(
        'EXPO_PUBLIC_GRID_ENV must be either "sandbox" or "production".'
      );
    }
