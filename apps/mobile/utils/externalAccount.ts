...
export class ExternalAccountService {
  ...
  static async storeAccount(
    ...
    try {
      ...
      if (existingStorage) {
        ...
        alreadyExists = storage.accounts.some(
          (acc) => acc.external_account_id === externalAccountId
        );
      } else {
        storage = { accounts: [] };
      }
      ...
      Sentry.captureException(
        new Error(
          `Error storing external account: ${error}. (utils)/externalAccount.ts (storeAccount) Label: ${label}`
        )
      );
  ...
  static async getAccountId(gridUserId: string): Promise<string | null> {
    try {
      ...
      const mapping = storage.accounts.find(
        (acc) => acc.grid_user_id === gridUserId
      );
      ...
      Sentry.captureException(
        new Error(
          `Error getting external account: ${error}. (utils)/externalAccount.ts (getAccountId)`
        )
      );
  ...
  static async getAllAccounts(): Promise<ExternalAccountStorage | null> {
    try {
      ...
      Sentry.captureException(
        new Error(
          `Error getting external accounts: ${error}. (utils)/externalAccount.ts (getAllAccounts)`
        )
      );
  ...
  static async deleteAccount(
    ...
    try {
      ...
      Sentry.captureException(
        new Error(
          `Error deleting external account: ${error}. (utils)/externalAccount.ts (deleteAccount)`
        )
      );
  ...
  static async clearAll(): Promise<void> {
    try {
      ...
      Sentry.captureException(
        new Error(
          `Error clearing external accounts: ${error}. (utils)/externalAccount.ts (clearAll)`
        )
      );
