...
export class StorageService {
  ...
  static async setItem(
    ...
    try {
      ...
      Sentry.captureException(
        new Error(
          `Error setting item ${key}: ${error}. (utils)/storage.ts (setItem)`
        )
      );
      throw error;
    }
  ...
  static async getItem<T>(key: string): Promise<T | null> {
    try {
      ...
      Sentry.captureException(
        new Error(
          `Error getting item ${key}: ${error}. (utils)/storage.ts (getItem)`
        )
      );
      return null;
    }
  ...
  static async deleteItem(key: string): Promise<void> {
    try {
      ...
      Sentry.captureException(
        new Error(
          `Error deleting item ${key}: ${error}. (utils)/storage.ts (deleteItem)`
        )
      );
      throw error;
    }
  ...
  static async hasItem(key: string): Promise<boolean> {
    try {
      ...
      Sentry.captureException(
        new Error(
          `Error checking item ${key}: ${error}. (utils)/storage.ts (hasItem)`
        )
      );
      return false;
    }
