...
export interface PreparedChainTransfer {
  unsignedTxBase64: string;
  /** Mirrors the backend's field so callers can treat both sources alike. */
  intentId: string;
}

/**
 * Builds an SPL transfer client-side, mirroring what the backend's
 * `prepareTransfer` produces: create the recipient's associated token account
 * if it is missing, then a checked transfer, on a v0 message with the sender as
 * fee payer.
 *
 * The sender pays their own fees on this path (no relayer), which is what makes
 * a backend-free Send possible at all.
 */
export async function prepareTransferOnChain(params: {
  fromAddress: string;
  toAddress: string;
  mint: string;
  amountRaw: string;
  decimals: number;
}): Promise<PreparedChainTransfer> {
  const {
  ...
  const instructions = [];
  // The sender pays the rent when the recipient has no token account yet.
  const recipientAta = await connection.getAccountInfo(toAta);
  if (!recipientAta) {
    instructions.push(
      createAssociatedTokenAccountInstruction(
        fromPk,
  ...
  return {
    ...
    // Marks the transfer as chain-built, so the submit path knows to broadcast
    // directly rather than handing an unknown id to the backend.
    intentId: CHAIN_INTENT_ID,
  };
}

/** Sentinel marking a transfer that was built client-side. */
export const CHAIN_INTENT_ID = "chain-direct";

/** Broadcasts a signed transaction straight to RPC. Returns the signature. */
export async function submitTransferOnChain(
