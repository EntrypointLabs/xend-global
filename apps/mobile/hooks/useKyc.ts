...
export function useKyc(): UseKycReturn {
    ...
    async (params: KycParams) => {
      ...
      try {
        ...
        Sentry.captureException(
          new Error(
            `Failed to start KYC process: ${err}. (hooks)/useKyc.ts (startKyc)`
          )
        );
  ...
  const checkStatus = useCallback(async () => {
    ...
    try {
      ...
      const response = await easClient.getKYCStatus(
        user.address,
        userData.kyc_link_id
      );

      const tosStatus =
        process.env.EXPO_PUBLIC_GRID_ENV === "production"
          ? (response.data.tos_status as TosStatus)
          : "approved";
      ...
      Sentry.captureException(
        new Error(
          `Failed to check KYC status: ${err}. (hooks)/useKyc.ts (checkStatus)`
        )
      );
  ...
  const getBankDescription = (type: "send" | "receive") => {
    if (isLoading || isBankLoading || !status) {
      ...
      return type === "send"
        ? "Send USDC to your Bank Account"
