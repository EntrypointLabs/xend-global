...
- "Wallet" appears in some UI copy (e.g. Spending Limits help text: "your wallet can handle...", `apps/mobile/app/(tabs)/settings/spending-limits.tsx`), flagged as a mistake to fix. The term is **Account**.
...
- "History" appears in code as a route filename, type, and screen name (`apps/mobile/app/(tabs)/history.tsx`, `apps/mobile/types/History.tsx`, `HistoryScreen`), flagged as legacy naming. The user-facing concept is **Activity** / **Activities** (matches the tab header and `ActivityList` component). Rename targets when touched: `activity.tsx`, `types/Activity.ts`, `ActivityScreen`.
- "Restore" vs "Recover", resolved in the routes: `(auth)/restore-account.tsx` is gone and the route is `(auth)/recover.tsx`. `settings/restore-device.tsx` keeps "restore" because it names the phone-side action of a **Recover**, not the concept.
- User-facing copy that says "balance" should mean the number. The surface is **Cash**; the number on it is **Balance**.
